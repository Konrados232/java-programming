...
class HashMap2D<R, C, V> implements Map2D<R, C, V>  {

    final private Map<R,Map<C,V>> mainMap = new HashMap<>();
    ...
    public V put(R rowKey, C columnKey, V value) {
        ...
        Map<C,V> columnMap = mainMap.get(rowKey);

        if (columnMap == null)
            columnMap = new HashMap<>();

        mainMap.put(rowKey, columnMap);
   ...
   /*     Map<C,V> columnMap = mainMap.get(rowKey);
    ...
    public V getOrDefault(R rowKey, C columnKey, V defaultValue) {
        ...
        if (columnMap == null)
            ...
            return columnMap.getOrDefault(columnKey,defaultValue);
    }
    ...
    public int size() {
        int size = 0;
        for (var columnMaps : mainMap.values()) {
            size += columnMaps.size();
        }
    ...
    public Map<C, V> rowView(R rowKey) {
        if (!hasRow(rowKey))
            return Collections.unmodifiableMap(new HashMap<C,V>()) ;
        else
            return Map.copyOf(mainMap.get(rowKey));
    }

    @Override
    public Map<R, V> columnView(C columnKey) {
        Map<R,V> newMap = new HashMap<R,V>();

        for (var map : mainMap.entrySet()) {
            var rowKey = map.getKey();
            var columnMap = map.getValue();
            var columnValue = columnMap.get(columnKey);

            if (columnMap.containsKey(columnKey))
                newMap.put(rowKey, columnValue);
        }
    ...
    public boolean hasValue(V value) {
        for (var columnMap : mainMap.values()) {
    ...
    public boolean hasKey(R rowKey, C columnKey) {
        return hasRow(rowKey) && hasColumn(columnKey);
    }

    @Override
    public boolean hasRow(R rowKey) {
        return mainMap.containsKey(rowKey);
    }
    ...
    public Map<R, Map<C, V>> rowMapView() {
        if (isEmpty()) {
            ...
            var newMap = new HashMap<R, Map<C, V>>();

            for (var rowKey : mainMap.keySet()) {
                newMap.put(rowKey, rowView(rowKey));
            }
    ...
    public Map<C, Map<R, V>> columnMapView() {
        if (isEmpty()) {
            ...
            Map<C,Map<R,V>> newMap = new HashMap<>();

            for (var columnMap : mainMap.values()) {
                for (var columnKey : columnMap.keySet()) {
                    newMap.put(columnKey, columnView(columnKey));
                }
            }
    ...
    public Map2D<R, C, V> putAll(Map2D<? extends R, ? extends C, ? extends V> source) {
        var x = source.rowMapView();
        mainMap.putAll((Map<? extends R, ? extends Map<C, V>>) x);
    ...
    public Map2D<R, C, V> putAllToRow(Map<? extends C, ? extends V> source, R rowKey) {
        for (var columnMap : source.entrySet()) {
            put(rowKey, columnMap.getKey(), columnMap.getValue());
        }
    ...
    public Map2D<R, C, V> putAllToColumn(Map<? extends R, ? extends V> source, C columnKey) {
        for (var rowMap : source.entrySet()) {
            put(rowMap.getKey(), columnKey, rowMap.getValue());
        }
    ...
    public <R2, C2, V2> Map2D<R2, C2, V2> copyWithConversion(Function<? super R, ? extends R2> rowFunction, Function<? super C, ? extends C2> columnFunction, Function<? super V, ? extends V2> valueFunction) {
        var newMap = new HashMap2D<R2,C2,V2>();

        for (var map : mainMap.entrySet()) {
            for (var columnMap : map.getValue().entrySet()) {
                var rowKey = map.getKey();
                var columnKey = columnMap.getKey();
                var value = columnMap.getValue();
                newMap.put((R2)rowFunction.apply(rowKey), (C2)columnFunction.apply(columnKey), (V2)valueFunction.apply(value));
            }
        }
        return newMap;
    }
